...
import { getLanguageName, normalizeJudgeOutput, pollBatchResults, submitBatch } from "@/lib/judge0";
...
export const executeCode = async (
  ...
  const detailedResults = results.map((result, i) => {
    ...
    const passed =
      stdout != null &&
      normalizeJudgeOutput(stdout) === normalizeJudgeOutput(expected_output);
  ...
  const submission = await prisma.submission.create({
    data: {
      ...
      status: allPassed
        ? "Accepted"
        : detailedResults.some((result) => result.stderr || result.compile_output)
          ? "Runtime Error"
          : "Wrong Answer",
